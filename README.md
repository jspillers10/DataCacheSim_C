...
ParameterMinimumMaximumConstraintSets18,192Power of 2Associativity18Any positive integerLine Size8 bytes64 bytesPower of 2
Compilation & Usage
Compilation
bashgcc -std=c99 -Wall -Wextra -O2 -o cache_simulator cache_simulator.c
Configuration File
...
bash./cache_simulator < trace_file.txt
Output Format
Per-Access Output
...
cunsigned int offset = address & ((1 << offset_bits) - 1);
...
cvoid update_lru(CacheLine *set, int associativity, int accessed_way) {
...
Corrected bit manipulation for proper tag/index/offset extraction
Implemented complete LRU replacement policy with counter-based tracking
Fixed memory reference counting to accurately track reads and writes
Improved input validation with comprehensive parameter checking
Restructured code into modular, well-documented functions
Corrected output formatting and removed debug statements
Added memory safety checks for allocation verification and proper cleanup
